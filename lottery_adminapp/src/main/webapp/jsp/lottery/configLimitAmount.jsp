...
<body>
	...
	<div class="container">
		...
		<form action="${contextPath}/lottery/savePlayRelated.do" name="lotteryForm" method="post">
		...
		<ul class="nav nav-tabs">
			<c:forEach items="${playModelList}" varStatus="status" var="pm">
				<c:if test="${status.index==0 }"><li class="active playclass""><a href="#" onclick="limitAmount.showLimitAmount(this);" name="play_div_${pm.id }"></c:if>
				<c:if test="${status.index!=0 }"><li class="playclass" ><a href="#" onclick="limitAmount.showLimitAmount(this);" name="play_div_${pm.id }"></c:if>
				${pm.modelName}&nbsp;</a>
				</li>
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						<a href="javascript:limitAmount.submitForm();" class="btn btn-primary"> 提交保存 </a> <a
							href="${contextPath }/lottery/showModifylpb.do" class="btn btn-link btn-sm">返回</a>
					</div>
				</div>
			</div>
		</div>
