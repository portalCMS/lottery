	...
	var initConfigDOM = function(data){
		...
		for(var i =0;i<playList.length;i++){
			...
			for(var j=0;j<lpmList.length;j++){
				...
				if(obj.group.startsWith("组选")){
					specialModel.push(lpmList[j]);
					continue;
				}
				//获取玩法
			...
			if(specialModel.length>0){
				...
				objGroup.group = "组选";
				var specialList = [];
				for(var p=0;p<specialModel.length;p++){
					//获取玩法
					var selectList = specialModel[p].selectList;
					//多奖级中奖金额
					var levelList = specialModel[p].levelList;
					var oriWinAmountStr = "";
					for(var k=0;k<levelList.length;k++){
						if(oriWinAmountStr==""){
							oriWinAmountStr = levelList[k].winAmount;
					...
					for(var x=0;x<selectList.length;x++){
						...
						if(selectList[x].selectName.indexOf("不含对子")>-1){
							...
							for(var t=0;t<descArr.length-1;t++){
									...
									spanStr2 = '</span>元';
									tempDesc = tempDesc + descArr[t]+spanStr
												+levelList[t].winAmount*specialModel[p].bonus.payoutRatio+spanStr2;
							}
						}
						...
						var str = selectList[x].selectIndex.split("_");
						selectObj.pos = [str[0],str[1]];
						selectObj.dataCode = specialModel[p].playModel.modelCode;
						selectObj.dataSelect = selectList[x].selectCode;
						selectObj.sumAmount = specialModel[p].playModel.winAmount*specialModel[p].bonus.payoutRatio;
						selectObj.winAmount = oriWinAmountStr;
						selectObj.payoutRatio = specialModel[p].bonus.payoutRatio;
						specialList.push(selectObj);
					}
				}
				objGroup.data=specialList;
