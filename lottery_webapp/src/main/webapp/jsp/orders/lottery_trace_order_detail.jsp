			...
			<div class="pageFrBox_1">
				<div class="clearfix p10 ">
					<ul class="fl clearfix">
						<li class="fl mr10"><a href="${fromUrl }" class="color_blue underline">追号记录</a>
						</li>
						<li class="fl mr10">></li>
				...
				<div class="p10 order_detail">
					...
					<ul class="p20">
						...
						<li>发起时间：<span class="color_8d"><fmt:formatDate value="${order.createTime }" pattern="yyyy-MM-dd HH:mm:ss" /> </span>
						</li>
						<li>当前状态：
							<c:if test="${order.rsvdc1 == order.rsvdc2 }">
								<span class="correctHint">完成</span>
							</c:if>
							<c:if test="${order.rsvdc1 != order.rsvdc2 }">
								<span class="correctHint">进行中</span>
							</c:if>
						</li>
						<li>中奖后终止：
							<c:if test="${order.awardStop == 1 }">
								<span class="correctHint">是</span>
							</c:if>
							<c:if test="${order.awardStop != 1 }">
								<span>否</span>
							</c:if>
